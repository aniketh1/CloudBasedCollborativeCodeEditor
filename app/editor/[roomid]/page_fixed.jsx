...
export default function EditorRoomPage() {
  ...
  useEffect(() => {
    ...
    socket.on('file-structure-update', (updatedFiles) => {
  ...
  useEffect(() => {
    if (mounted) {
      socket.emit('file-structure-change', { roomId, files });
    }
  }, [files, mounted, roomId]);
    ...
    <div className="min-h-screen bg-[#1a1a2e] text-white flex flex-col">
      ...
      <div className="bg-[#16161e] h-8 flex justify-between items-center px-2 border-b border-gray-800">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-300">Collab Dev - Room {roomId}</span>
        </div>
        <div className="flex items-center space-x-2">
